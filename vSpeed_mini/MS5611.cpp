...
//          Braedin Butler - tuned for vSpeed Vario
// VERSION: 0.1.10
// PURPOSE: MS5611 Temperature & Atmospheric Pressure library for Arduino
//     URL:
//
// POST FORK HISTOTY
// 0.1.10 2020-09-12 Tuned for speed and tuned OSRs to vSpeed Vario preferences.
...
int MS5611::read(/*uint8_t bits*/)
{
  ...
  convert(0x40, 12); // change 12 to bits if you want to go back to custom
  uint32_t D1 = readADC();

  convert(0x50, 9); // change 9 to bits if you want to go back to custom
  uint32_t D2 = readADC();
...
void MS5611::convert(const uint8_t addr, uint8_t bits)
{
  ...
  wait(bits-7);                                           // improved delay times for faster speed
  digitalWrite(_cspin,HIGH);                            // pull CS line high
  SPI.endTransaction();                                 // end SPI transaction
}

void MS5611::wait(uint8_t OSR_mode){
  ...
  switch(OSR_mode){
    ...
    default://(OSR_4096 @ 8.22ms)
      //OSR = 0x48;
      DLY = 8420;//8220; // NOT SURE WHY 8220 WONT WORK ANYMORE
      break;
  }
